...
class SignalItem(BaseModel):
    ...
    expires_in_sec: Optional[int] = None  # int | None (백엔드 SignalIn 과 동일)


class PredictionItem(BaseModel):
    """
    백엔드 PredictionIn 과 동일.
    predicted_price_human / lower_bound_human / upper_bound_human 은
    to_base_units(value, asset.decimals) 로 변환되어 저장됨.
...
class SentimentItem(BaseModel):
    ...
    extra_json: Optional[str] = None  # JSON 문자열 (백엔드 그대로 저장)
...
class AIInferenceResult(BaseModel):
    ...
    한 symbol 에 대한 전체 AI 추론 결과.
      ...
      sentiment : sentiment_score, rsi, macd, ma7_human, ma25_human
    """
    ...
    macd: Optional[float] = None
    ma7_human: Optional[str] = None  # 캔들 close 로 계산한 실제 MA7
    ma25_human: Optional[str] = None  # 캔들 close 로 계산한 실제 MA25
